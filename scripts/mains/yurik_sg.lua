...
local CHANGE_EQUIP = Action({ priority = 10, mount_valid = false })
      ...
      CHANGE_EQUIP.str = "切换"   
      CHANGE_EQUIP.fn = function(act) --这个操作执行时进行的功能函数
...
ACTIONS.CASTAOE.strfn = function(act)
    if act.doer and act.doer:HasTag("yurri") and act.doer._skill:value() and act.invobject and act.invobject.prefab == "yurik_camera" then  --print(ThePlayer._skill:value())
